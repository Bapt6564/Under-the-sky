...
const SUJETS = {
  ...
  "antares-scorpion": {
    ...
    sections: [
      `<h2>Repérage</h2>
       <p>En été, cherchez bas sur l'horizon sud une longue courbe d'étoiles en <strong>hameçon</strong> : le Scorpion, l'une des rares constellations qui ressemble vraiment à son nom. Antarès brille au cœur, avec une couleur rouge-orangé nette même à l'œil nu.</p>`,
      `<h2>Chiffres clés</h2>
       <div class="chiffre-cle">Distance : ~550 années-lumière · Température : seulement ~3 400 °C (d'où le rouge) · Taille : ~700 fois le rayon du Soleil — si elle remplaçait le Soleil, elle engloberait l'orbite de Mars.</div>`,
      `<h2>Anecdotes</h2>
       <ul>
         <li>Son nom signifie <strong>« rivale d'Arès/Mars »</strong> : les astronomes grecs comparaient déjà sa couleur rouge à celle de la planète.</li>
         <li>C'est une supergéante en toute fin de vie : elle explosera un jour en supernova (<a href="#/memo/cycle-vie-etoiles">cycle de vie des étoiles</a>) — comme sa cousine Bételgeuse dans Orion.</li>
       </ul>`
    ],
